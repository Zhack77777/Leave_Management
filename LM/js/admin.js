...
function loadReports() {
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-3" id="reportPeriodCol">
                            ...
                            <div class="col-md-3" id="reportDateCol">
                                ...
                                <input type="month" class="form-control d-none" id="reportMonth">
                                <input type="number" class="form-control d-none" id="reportYear" min="2000" max="2100" value="${new Date().getFullYear()}">
                            </div>
                        ...
                        <div id="reportOutput">
                            <p>Select a report type and period, then click Generate Report.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // Show/hide date/month/year inputs based on period
    ...
    function updateDateInputs() {
        reportDate.classList.add('d-none');
        reportMonth.classList.add('d-none');
        reportYear.classList.add('d-none');
        if (reportPeriod.value === 'day') {
            reportDate.classList.remove('d-none');
        } else if (reportPeriod.value === 'month') {
            reportMonth.classList.remove('d-none');
    ...
    reportPeriod.addEventListener('change', updateDateInputs);
    updateDateInputs();
}

// Update generateReport to handle attendance report and period selection
async function generateReport() {
    ...
    try {
        ...
        if (reportType === 'attendance') {
            headers = ['Name', 'Department', 'Type', 'Time', 'Date', 'Address'];
            ...
            if (reportPeriod === 'day') {
                ...
                const start = new Date(d.setDate(diffToSunday));
                const end = new Date(start);
                end.setDate(start.getDate() + 6);
                startDate = start.toISOString().split('T')[0];
                endDate = end.toISOString().split('T')[0];
            }
            const [clockSnap, usersSnap] = await Promise.all([
                database.ref('time_clock').once('value'),
            ...
            clockSnap.forEach(clockSnap => {
                ...
                if (clockDate < startDate || clockDate > endDate) return;
                const userData = users[clock.userId];
                if (!userData) return;
                data.push([
                    userData.name,
                    userData.department,
                    clock.type === 'in' ? 'Clock In' : 'Clock Out',
                    dateObj.toLocaleTimeString(),
                    clockDate,
                    clock.address || `${clock.latitude || ''}, ${clock.longitude || ''}`
                ]);
            });
        } else if (reportType === 'leaveBalances') {
        ...
        if (data.length === 0) {
            reportOutput.innerHTML = '<p>No data found for the selected report type and period.</p>';
            return;
        }
        ...
        if (reportType === 'attendance') {
            if (reportPeriod === 'day') {
                exportPeriodLabel = reportDate;
            } else if (reportPeriod === 'month') {
                exportPeriodLabel = reportMonth;
            } else if (reportPeriod === 'year') {
                exportPeriodLabel = reportYear;
            } else if (reportPeriod === 'week') {
                exportPeriodLabel = startDate + '_to_' + endDate;
            }
        } else {
            exportPeriodLabel = reportYear || '';
        }
        let tableHtml = `
            ...
            <button class="btn btn-success mt-3" onclick="exportToCSV('${reportType}', '${exportPeriodLabel}')">Export to CSV</button>
        `;
...
function exportToCSV(reportType, periodLabel) {
    const reportOutput = document.getElementById('reportOutput');
    ...
    link.setAttribute('download', `${reportType}_${periodLabel}.csv`);
